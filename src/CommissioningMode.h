...
struct CommissioningAxisMeta
{
    ...
    // NOTE (decided 0.9.4): no free-form per-axis mixing weights. The corner
    // roles describe four-post rigs; anything finer is kinematics, which
    // nullCAT deliberately does not do -- a wrong hand-typed weight on a
    // coupled platform binds the mechanism. Hexapod legs take part in heave
    // (roles unset) and are characterised per leg; platform pitch/roll
    // belongs to the motion software that owns the kinematics.
};
